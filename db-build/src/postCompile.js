...
function postCompile(agencyOptions, db, log, errors, callback) {
  ...
  if ( agencyOptions.compile === true ) {
    ...
    db.serialize(function() {
      ...
      console.log("    ... Updating SLE Trip Directions");
      db.exec("UPDATE gtfs_trips SET direction_id=0 WHERE route_id LIKE 'SLE%' AND trip_headsign IN ('To New London', 'To Old Saybrook', 'To Guilford', 'To Clinton');");
      db.exec("UPDATE gtfs_trips SET direction_id=1 WHERE route_id LIKE 'SLE%' AND trip_headsign IN ('To New Haven Union Station', 'To New Haven State Street Station', 'To Stamford');");
